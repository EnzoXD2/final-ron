...
class StoryMenuState extends MusicBeatState
{
	...
	var grpWeekCharacters:FlxTypedGroup<gameassets.MenuCharacter>;
	...
	override function create()
	{
		...
		grpWeekCharacters = new FlxTypedGroup<gameassets.MenuCharacter>();
		...
		for (char in 0...3)
		{
			var weekCharacterThing:gameassets.MenuCharacter = new gameassets.MenuCharacter((FlxG.width * 0.25) * (1 + char) - 150, charArray[char]);
	...
	function selectWeek()
	{
		if (!weekIsLocked(loadedWeeks[curWeek].fileName))
		{
			...
			if (stopspamming == false)
			{
				...
				var bf:gameassets.MenuCharacter = grpWeekCharacters.members[1];
